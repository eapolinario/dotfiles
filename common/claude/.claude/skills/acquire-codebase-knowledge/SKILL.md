...
license: MIT
compatibility: 'Cross-platform. Requires uv and git. uv manages the Python runtime for scripts/scan.py. Run scripts/scan.py from the target project root.'
metadata:
  version: "1.3"
  enhancements:
...
1. Run the scan script from the target project root:
   ```bash
   uv run "$SKILL_ROOT/scripts/scan.py" --output docs/codebase/.codebase-scan.txt
   ```
   ...
   **Quick start:** If you have the path inline:
   ```bash
   uv run /absolute/path/to/skills/acquire-codebase-knowledge/scripts/scan.py --output docs/codebase/.codebase-scan.txt
   ```
...
| Asset | When to load |
|-------|-------------|
| [`scripts/scan.py`](scripts/scan.py) | Phase 1 — run first with uv, before reading any code |
