...
- =omarchy/= — omarchy extension overrides (=~/.config/omarchy/extensions/=)
...
- =configuration.nix= — system-level packages and services
- =home/eduardo/= — user environment (packages, waybar, gpg-agent, xremap,
  cliphist, nushell, fzf, zoxide)
