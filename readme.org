...
Also untracked on purpose: =~/.config/starship.toml= is Omarchy's shipped
prompt. Starship has no include mechanism, so tracking it would mean vendoring
the whole upstream file to carry a local tweak.
